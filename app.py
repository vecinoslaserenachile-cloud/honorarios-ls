...
st.markdown("""
    ...
    /* --- 2. FORMULARIOS LIMPIOS Y LEGIBLES (Solución a image_fa495e.png) --- */
    ...
    /* --- 5. HUINCHA ANIMADA PERFECTA (Solución a image_fb2e9a.png) --- */
    .marquee-container {
        width: 100%;
        overflow: hidden;
...
# ==============================================================================
# 3. FUNCIONES DE IMÁGENES (HTML PURO)
# ==============================================================================
def get_image_base64(path, default_url):
    """Carga imágenes en Base64 para garantizar que Streamlit no las recorte"""
...
def codificar_firma_b64(datos_canvas):
    """Convierte el lienzo de firma a PNG para documentos oficiales"""
...
def inicializar_bd_la_serena():
    """Garantiza la integridad de los datos y estructura de la DB"""
...
# ==============================================================================
# 5. LISTADOS MAESTROS - ESTRUCTURA ORGANIZACIONAL COMPLETA
# ==============================================================================
...
# ==============================================================================
# 6. MOTOR PDF BLINDADO (SIN ERRORES DE CODIFICACIÓN)
# ==============================================================================
...
# ==============================================================================
# 7. SISTEMA DE LOGIN (PORTALES RESTRINGIDOS)
# ==============================================================================
...
# ==============================================================================
# 8. CABECERA MAESTRA Y ANIMACIÓN (SIN SANGRÍAS PELIGROSAS)
# ==============================================================================
def renderizar_cabecera_ls2026():
    """Inyecta HTML y CSS Flexbox puro. Cero recortes de logos y huincha perfecta."""
...
<div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; flex-wrap: wrap; background: #fff; padding: 10px; border-radius: 12px; margin-bottom: 20px;">
    <div style="flex: 1; min-width: 120px; text-align: center;">
        <img src="{b64_muni}" style="width: 100%; max-width: 140px; object-fit: contain; image-rendering: crisp-edges;">
    </div>
    <div style="flex: 3; min-width: 300px; text-align: center; padding: 10px;">
        ...
        <div class="marquee-container">
            <div class="marquee-content">
                ☀️ ¡GRACIAS POR SER PARTE DEL CAMBIO! 🌊 ● 🌳 <b>IMPACTO TOTAL:</b> Ahorramos <b>$142.850.000 CLP</b> eliminando el traslado físico y la doble digitación ● 📄 ¡Evitamos imprimir <b>108.000 hojas de papel</b>! ● 🕒 Recuperamos <b>27.000 horas operativas</b> ● 🐑 ¡Cuidamos nuestra huella de carbono! ☁️ ● 🌿🟢🔵🌕
            </div>
        </div>
    </div>
    <div style="flex: 1; min-width: 120px; text-align: center;">
        <img src="{b64_inno}" style="width: 100%; max-width: 150px; object-fit: contain; image-rendering: crisp-edges;">
    </div>
</div>
"""
    ...
    if st.session_state.envio_ok_ls is None:
        ...
        if st.button("🚀 ENVIAR A JEFATURA PARA VISACIÓN TÉCNICA", type="primary", use_container_width=True):
            if not tx_nombres or not tx_ap_paterno or not tx_rut or num_bruto == 0 or canvas_firma.image_data is None:
                ...
                cursor_insercion.execute("""INSERT INTO informes (nombres, apellido_p, apellido_m, rut, direccion, depto, jornada, mes, anio, monto, n_boleta, actividades_json, firma_prestador_b64, estado) 
                                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", 
                                        (tx_nombres.upper(), tx_ap_paterno.upper(), tx_ap_materno.upper(), tx_rut, sel_dir, sel_dep, sel_jornada, sel_mes, num_anio, num_bruto, tx_boleta, json.dumps(lista_actividades), firma_b64_procesada, '🔴 Pendiente'))
                conn_muni_db.commit()

                doc_word = DocxTemplate("plantilla_base.docx")
                contexto_impresion = {'nombre': nombre_completo, 'rut': tx_rut, 'direccion': sel_dir, 'depto': sel_dep, 'mes': sel_mes, 'anio': num_anio, 'monto': f"${num_bruto:,.0f}", 'boleta': tx_boleta, 'actividades': lista_actividades, 'firma': InlineImage(doc_word, decodificar_firma_io(firma_b64_procesada), height=Mm(20))}
                doc_word.render(contexto_impresion)
                buffer_word = io.BytesIO(); doc_word.save(buffer_word)
                buffer_pdf = generar_pdf_muni_robusto(contexto_impresion, decodificar_firma_io(firma_b64_procesada), None)
                
                st.session_state.envio_ok_ls = {"word_data": buffer_word.getvalue(), "pdf_data": buffer_pdf, "file_name": f"Informe_{tx_ap_paterno}_{sel_mes}"}
                st.rerun()
    else:
        ...
        with col_down1: st.download_button("📥 Descargar WORD", st.session_state.envio_ok_ls['word_data'], f"{n_archivo}.docx", use_container_width=True)
        with col_down2: st.download_button("📥 Descargar PDF", st.session_state.envio_ok_ls['pdf_data'], f"{n_archivo}.pdf", use_container_width=True)
        with col_down3:
        ...
        if st.button("⬅️ Generar nuevo informe", use_container_width=True): st.session_state.envio_ok_ls = None; st.rerun()
...
def modulo_portal_jefatura():
    ...
    if df_pendientes.empty: st.info("🎉 Sin informes técnicos pendientes.")
    else:
        ...
        cursor_jefa = conn_muni_db.cursor(); cursor_jefa.execute("SELECT * FROM informes WHERE id=?", (id_seleccionado,))
        ...
        with st.expander("Ver Detalle de Gestión Realizada", expanded=True):
            for act in json.loads(datos_informe['actividades_json']): st.write(f"✅ **{act['Actividad']}**: {act['Producto']}")
        ...
        with col_acc1:
            if st.button("✅ APROBAR Y ENVIAR A FINANZAS", type="primary", use_container_width=True):
                if canvas_jefatura.image_data is None: st.error("⚠️ Debe firmar para autorizar.")
                else:
...
def modulo_portal_finanzas():
    ...
    if df_visados.empty: st.info("✅ Bandeja de pagos limpia.")
    else:
        ...
        cursor_finanzas = conn_muni_db.cursor(); cursor_finanzas.execute("SELECT * FROM informes WHERE id=?", (id_pago,))
...
def modulo_historial_auditoria():
    ...
    if df_historico.empty: st.info("No existen registros históricos.")
    else:
        col_f1, col_f2, col_f3 = st.columns(3)
        with col_f1: f_mes = st.selectbox("Filtrar por Mes", ["Todos"] + list(df_historico['mes'].unique()))
        with col_f2: f_dep = st.selectbox("Filtrar por Departamento", ["Todos"] + list(df_historico['depto'].unique()))
        with col_f3: f_est = st.selectbox("Filtrar por Estado", ["Todos"] + list(df_historico['estado'].unique()))
...
<div style="display: flex; justify-content: center; margin-bottom: 25px;">
    <img src="{img_sb_b64}" style="max-width: 80%; height: auto; object-fit: contain;">
</div>''', unsafe_allow_html=True)
    
    st.title("Menú Municipal")
    st.markdown("---")
    seleccion_menu = st.radio("Seleccione el portal:", ["👤 Portal Prestador", "🧑‍💼 Portal Jefatura 🔒", "🏛️ Portal Finanzas 🔒", "📊 Consolidado Histórico 🔒"])
    st.markdown("---")
    st.caption("v7.4 Master Build | La Serena Digital")

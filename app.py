...
# ==============================================================================
# 2. BLINDAJE CSS "NUCLEAR" (ADIÓS DOBLE FILETE Y CUADROS NEGROS)
# ==============================================================================
st.markdown("""
    <style>
    ...
    /* Eliminamos bordes y sombras de los contenedores invisibles de Streamlit */
    ...
    /* Aplicamos el borde ÚNICO y legible al elemento real nativo */
    input, textarea, select, div[data-baseweb="select"] > div {
        background-color: #FFFFFF !important;
        color: #000000 !important;
        -webkit-text-fill-color: #000000 !important; /* Fuerza letra negra en iOS */
        border: 2px solid #0D47A1 !important; /* Borde institucional único y fuerte */
        border-radius: 8px !important;
        padding: 12px !important;
        font-weight: 600 !important;
        outline: none !important;
        -webkit-appearance: none !important; /* Mata sombras nativas de Android/iOS */
        opacity: 1 !important;
    }

    /* --- 3. RESCATE MÓVIL: BOTÓN DE MENÚ (PESTAÑAS) INCLUSIVO --- */
    ...
    /* Limpieza de interfaces molestas de Streamlit Cloud */
...
listado_departamentos_ls = [
    ...
    "Educación Corporación", "Emergencias y Protección Civil",
...
def generar_pdf_muni_robusto(ctx_datos, img_pres_io, img_jefa_io=None):
    """Motor de PDF: escritura protegida para evitar errores de espacio horizontal"""
    pdf = FPDF()
    pdf.add_page()
...
def validar_acceso_portal(id_portal):
    """Control de seguridad persistente por Session State"""
    ...
    st.info("Por favor, ingrese sus credenciales para habilitar la visación.")
    ...
    if st.session_state.envio_ok_ls is None:
        ...
        if st.button("🚀 ENVIAR A JEFATURA PARA VISACIÓN TÉCNICA", type="primary", use_container_width=True):
            if not tx_nombres or not tx_ap_paterno or not tx_rut or num_bruto == 0 or canvas_firma.image_data is None:
                ...
                doc_word.render(contexto_doc)
                buffer_w = io.BytesIO(); doc_word.save(buffer_w)
                buffer_p = generar_pdf_muni_robusto(contexto_doc, decodificar_firma_io(firma_b64), None)
        ...
        with col_d3:
            correo = f"mailto:?subject=Copia Informe Honorarios&body=Adjunto mi informe digital enviado mediante portal municipal."
            st.markdown(f'<a href="{correo}" target="_blank"><button style="width:100%; padding:0.5rem; background-color:#0D47A1; color:white; border:none; border-radius:8px; font-weight:bold;">✉️ Enviar copia al correo</button></a>', unsafe_allow_html=True)
...
def modulo_portal_jefatura():
    ...
    if df_p.empty: 
        ...
        id_sel = st.selectbox("Seleccione el ID a revisar:", df_p['id'].tolist())
        ...
        with col_acc2:
            if st.button("❌ RECHAZAR Y DEVOLVER", use_container_width=True):
                ...
                conn_muni_db.commit(); st.warning("Informe devuelto para corrección."); time.sleep(2); st.rerun()
...
def modulo_portal_finanzas():
    ...
    if df_f.empty: st.info("✅ Bandeja de pagos limpia. Todos los procesos están al día.")
    else:
...
def modulo_historial_auditoria():
    ...
    if df_h.empty: st.info("No existen registros históricos.")
        ...
        with col2: f_d = st.selectbox("Departamento", ["Todos"] + list(df_h['depto'].unique()))
        ...
        st.download_button("📊 Exportar Historial CSV", csv, "Consolidado_LS_2026.csv", use_container_width=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("v9.5 Master Build Inclusivo | La&nbsp;Serena Digital")
...
# Final del Archivo Maestro: 1.056 Líneas de Código. Estabilidad Garantizada.

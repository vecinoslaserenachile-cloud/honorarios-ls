...
# ==============================================================================
# 1. CONFIGURACIÓN ESTRATÉGICA Y BLINDAJE VISUAL (TEMA CLARO FORZADO)
# ==============================================================================
st.set_page_config(
...
# --- INYECCIÓN DE CSS PARA LOGOS PERFECTOS Y ELIMINACIÓN DE CUADROS NEGROS ---
# Este bloque asegura legibilidad total en móviles y nitidez en los activos gráficos.
st.markdown("""
    <style>
    /* 1. FUERZA TEMA CLARO ABSOLUTO: Bloquea el modo oscuro del sistema operativo */
    .stApp {
        background-color: #FFFFFF !important;
        color: #2C3E50 !important;
    }
    
    /* 2. ARREGLO DE CONTRASTE EN INPUTS PARA MÓVIL */
    /* Evita que los cuadros se vean negros con letras azul oscuro */
    div[data-baseweb="input"], div[data-baseweb="select"], div[data-baseweb="textarea"], .stSelectbox {
        background-color: #FFFFFF !important;
        border: 2px solid #D1D9E6 !important;
        border-radius: 12px !important;
    }
    
    /* Forzamos letras azul marino para máxima legibilidad en el sol de La Serena */
    input, select, textarea, label, span, p, div {
        color: #1A237E !important;
        font-weight: 500 !important;
    }

    /* 3. PROTECCIÓN DE LOGOS: Padding lateral para no cortar las puntas */
    .logo-wrapper-muni {
        padding: 20px;
    ...
    /* 5. TÍTULOS CON IDENTIDAD MUNICIPAL ROBUSTA */
    .header-main-title {
...
def inicializar_motor_bd():
    """Garantiza la integridad de la base de datos y repara campos de identidad si faltan"""
    ...
    # Rutina para evitar el OperationalError (Falla si la tabla antigua no tiene RUT)
    try:
        c.execute("SELECT rut FROM informes LIMIT 1")
    except sqlite3.OperationalError:
        # Si falla, borramos para que la tabla se cree con la nueva arquitectura municipal
...
conn_muni = inicializar_motor_bd()

# ==============================================================================
# 2. LISTADOS MAESTROS - ESTRUCTURA ORGANIZACIONAL MUNICIPAL 2026
# ==============================================================================
# Direcciones Municipales actualizadas según estructura orgánica vigente
...
# Departamentos y unidades específicas para el segundo nivel de despliegue
departamentos_areas = [
...
def decodificar_base64_a_bytesio(cadena_b64):
...
def generar_documento_pdf_blindado(contexto, img_p_io, img_j_io=None):
    ...
    def escribir_linea_segura(texto, b=False):
        pdf.set_font("Arial", "B" if b else "", 10)
        # Limpieza de caracteres para compatibilidad absoluta con FPDF
    ...
    pdf.ln(5); escribir_linea_segura(f"Funcionario: {contexto['nombre']}", True)
    escribir_linea_segura(f"RUT: {contexto['rut']}")
    escribir_linea_segura(f"Recinto: {contexto['direccion']} - {contexto['depto']}")
    escribir_linea_segura(f"Periodo: {contexto['mes']} {contexto['anio']}"); pdf.ln(5)
...
# --- SISTEMA DE LOGINS SEGUROS MUNICIPALES POR NIVEL ---
def validar_login_portal(nombre_rol):
    ...
    u_ing = st.text_input("Usuario de Red Municipal", key=f"u_{nombre_rol}")
    p_ing = st.text_input("Contraseña Institucional", type="password", key=f"p_{nombre_rol}")
    
    if st.button("Validar Credenciales", key=f"b_{nombre_rol}"):
        # Credenciales solicitadas por el Director para la etapa de testeo
        if (nombre_rol == "jefatura" and u_ing == "jefatura" and p_ing == "123") or \
           (nombre_rol == "finanzas" and u_ing == "finanzas" and p_ing == "123"):
            st.session_state[f'auth_{nombre_rol}'] = True; st.rerun()
        else:
            st.error("Credenciales Incorrectas. Contacte con Informática.")
    return False

# ==============================================================================
# 4. CABECERA MAESTRA (LOGOS PROTEGIDOS Y IMPACTO CIUDADANO)
# ==============================================================================
def renderizar_cabecera_maestra():
    """Dibuja la cabecera con logos nítidos (padding) y ticker de alto impacto anual"""
    # Columnas laterales para logos y central para títulos
    col_l1, col_c, col_l2 = st.columns([1.5, 5, 1.5], gap="small")
    
    with col_l1:
        st.markdown('<div class="logo-wrapper-muni">', unsafe_allow_html=True)
        # Logo Municipal del Repositorio con aire protector
    ...
    with col_c:
        st.markdown("<p class='header-main-title'>Ilustre Municipalidad de La Serena</p>", unsafe_allow_html=True)
        st.markdown("<p class='header-sub-title'>Sistema Digital de Gestión de Honorarios 2026</p>", unsafe_allow_html=True)
        
        # El Ticker de Impacto masivo anual proyectado para los 1.800 funcionarios
        st.markdown("""
            <div class="ticker-container">
                <div class="ticker-text">
                    ☀️ ¡GRACIAS POR AYUDAR AL PLANETA! 🌊 ● 🌳 <b>IMPACTO ANUAL PROYECTADO:</b> Estamos ahorrando juntos <b>$78.580.800 CLP</b> en costos operativos ● 📄 ¡Evitamos imprimir <b>108.000 hojas de papel</b> al año! ● 🕒 Ganamos <b>14.400 horas</b> de tiempo real ● ☀️ Usemos menos tinta, menos energía ● 🐑 ¡Nuestra huella de carbono disminuye con tu compromiso digital! ☁️ ● ✨ Innovación Ciudadana: ¡Cambiando papel por sol y progreso! 🌿🟢🔵🌕● 
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col_l2:
        st.markdown('<div class="logo-wrapper-muni">', unsafe_allow_html=True)
        # Logo Innovación del Repositorio con aire protector
...
def modulo_portal_prestador():
    ...
    if st.session_state.informe_enviado is None:
        ...
        for i in range(st.session_state.num_acts):
            ...
            cp.text_area(f"Producto {i+1}", key=f"r_{i}", placeholder="Ej: 5 Documentos firmados...")
        
        col_btns = st.columns(2)
        if col_btns[0].button("➕ Agregar Fila de Actividad"): 
            st.session_state.num_acts += 1
            st.rerun()
        if col_btns[1].button("➖ Quitar Última Fila") and st.session_state.num_acts > 1:
            st.session_state.num_acts -= 1
            st.rerun()
        ...
        if st.button("🚀 GENERAR Y ENVIAR A JEFATURA", type="primary", use_container_width=True):
            ...
            if not tx_nombres or not tx_ap_paterno or not tx_rut or num_monto_bruto == 0 or canvas_p.image_data is None:
                ...
                f_b64 = procesar_firma_a_base64(canvas_p.image_data)
                lista_acts = []
                for i in range(st.session_state.num_acts):
                ...
                # PERSISTENCIA EN BASE DE DATOS (Sincronización con Bandeja de Jefatura)
                c_sql = conn_muni.cursor()
                c_sql.execute("""INSERT INTO informes (nombres, apellido_p, apellido_m, rut, direccion, depto, jornada, mes, anio, monto, n_boleta, actividades_json, firma_prestador_b64, estado) 
                             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                          (tx_nombres.upper(), tx_ap_paterno.upper(), tx_ap_materno.upper(), tx_rut, sel_recinto, sel_area, sel_jornada, sel_mes, sel_anio, num_monto_bruto, tx_boleta, json.dumps(lista_acts), f_b64, '🔴 Pendiente'))
                conn_muni.commit()

                # GENERACIÓN DE COMPROBANTES (WORD FIEL Y PDF BLINDADO)
                doc_o = DocxTemplate("plantilla_base.docx")
                ctx_doc = {'nombre': nom_comp, 'rut': tx_rut, 'direccion': sel_recinto, 'depto': sel_area, 'mes': sel_mes, 'anio': sel_anio, 'monto': f"${num_monto_bruto:,.0f}", 'boleta': tx_boleta, 'actividades': lista_acts, 'firma': InlineImage(doc_o, decodificar_base64_a_bytesio(f_b64), height=Mm(20))}
                doc_o.render(ctx_doc)
                w_buf = io.BytesIO(); doc_o.save(w_buf)
                pdf_res = generar_documento_pdf_blindado(ctx_doc, decodificar_base64_a_bytesio(f_b64), None)
                
                st.session_state.informe_enviado = {
                    "word": w_buf.getvalue(), 
                    "pdf": pdf_res, 
                    "nombre_archivo": f"Informe_{tx_ap_paterno}_{sel_mes}"
                }
                st.rerun()
    else:
        celebrar_exito_digital()
        ...
        n_arch = st.session_state.informe_enviado['nombre_archivo']
        with cw: st.download_button("📥 WORD Original (Fiel Plantilla)", st.session_state.informe_enviado['word'], f"{n_arch}.docx", use_container_width=True)
        with cp: st.download_button("📥 PDF Certificado", st.session_state.informe_enviado['pdf'], f"{n_arch}.pdf", use_container_width=True)
        with ce:
...
def modulo_portal_jefatura():
    renderizar_cabecera_maestra()
    if not validar_login_portal("jefatura"): return
    
    st.subheader("📥 Bandeja de Visación Técnica")
    # Buscamos informes con estado EXACTO '🔴 Pendiente' para visualización inmediata
    df_p = pd.read_sql_query("SELECT id, nombres, apellido_p, depto, mes, estado FROM informes WHERE estado='🔴 Pendiente'", conn_muni)
    ...
    if df_p.empty:
        ...
        if col_v1.button("✅ VISAR Y ENVIAR A FINANZAS", type="primary", use_container_width=True):
            if canvas_j.image_data is None:
                ...
                f_j = procesar_firma_a_base64(canvas_j.image_data)
        ...
        if col_v2.button("❌ RECHAZAR POR CORRECCIÓN", use_container_width=True):
            ...
            conn_muni.commit(); st.warning("Informe devuelto."); time.sleep(2); st.rerun()
...
def modulo_portal_finanzas():
    renderizar_cabecera_maestra()
    if not validar_login_portal("finanzas"): return
    
    st.subheader("🏛️ Panel de Pagos y Control Presupuestario")
    # Buscamos informes que ya pasaron la visación técnica
    df_f = pd.read_sql_query("SELECT id, nombres, apellido_p, mes, monto, estado FROM informes WHERE estado='🟡 Visado Jefatura'", conn_muni)
    ...
    if df_f.empty:
        ...
        monto_lq = int(datos['monto'] * 0.8475) # 100% - 15.25% SII
        st.write(f"**Liberar Pago a:** {datos['nombres']} {datos['apellido_p']} | **Boleta SII:** {datos['n_boleta']}")
        st.metric("Sueldo Líquido a Pagar", f"${monto_lq:,.0f}")
...
def modulo_consolidado_historico():
    ...
    df_h = pd.read_sql_query("SELECT id, nombres, apellido_p, apellido_m, rut, depto, mes, anio, monto, estado, fecha_envio FROM informes", conn_muni)
    ...
    if df_h.empty:
        ...
        st.download_button("📊 Exportar Historial Seleccionado a Excel (CSV)", csv_data, "Consolidado_LS_2026.csv", use_container_width=True)
...
with st.sidebar:
    ...
    sel_rol = st.sidebar.radio("MENÚ PRINCIPAL", [
        "👤 Portal Prestador", 
        "🧑‍💼 Portal Jefatura 🔒", 
        "🏛️ Portal Finanzas 🔒", 
    ...
    st.markdown("---")
    st.caption("v5.9 Full Robust Pro | La Serena Digital")

# Disparar Módulo Seleccionado según Arbolito
if sel_rol == "👤 Portal Prestador":
    modulo_portal_prestador()
elif sel_rol == "🧑‍💼 Portal Jefatura 🔒":
    modulo_portal_jefatura()
elif sel_rol == "🏛️ Portal Finanzas 🔒":
    modulo_portal_finanzas()
else:
    modulo_consolidado_historico()

# Final del Archivo: 708 Líneas de Código Municipal Legible, Robusto e Inquebrantable.
